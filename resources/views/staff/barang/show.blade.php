...
@section('content')
    <div class="mb-6 lg:mb-8">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            @if(Auth::user()->hasPermission('manage_barang'))
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($barang->units as $index => $unit)
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-6 py-4">
                                @if(Auth::user()->hasPermission('manage_barang'))
                                    <form action="{{ route('staff.barang.updateUnit', $unit->id) }}" method="POST" class="inline">
                                        @csrf
                                        @method('PUT')
                                ...
                                @else
                                    <span class="px-3 py-1.5 text-sm {{ $unit->kondisi == 'Baik' ? 'text-green-600' : ($unit->kondisi == 'Rusak Ringan' ? 'text-yellow-600' : 'text-red-600') }}">
                                        {{ $unit->kondisi }}
                                    </span>
                                @endif
                            </td>
                            <td class="px-6 py-4">
                                @if($barang->tipe_barang == 'maintenance' && Auth::user()->hasPermission('manage_barang'))
                                    <form action="{{ route('staff.barang.updateUnit', $unit->id) }}" method="POST" class="inline">
                                        @csrf
                                        @method('PUT')
                                ...
                                @else
                                    <span class="px-3 py-1.5 text-sm {{ $unit->status == 'aktif' ? 'text-green-600' : 'text-red-600' }}">
                                        {{ $unit->status == 'aktif' ? 'Aktif' : ($unit->status == 'maintenance' ? 'Maintenance' : 'Rusak') }}
                                    </span>
                                @endif
                            </td>
