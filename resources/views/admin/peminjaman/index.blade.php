...
@section('content')
    ...
    <div class="bg-white p-4 rounded-xl border border-gray-100 mb-6">
        <form method="GET" action="{{ route('admin.peminjaman.index') }}" class="flex gap-4 w-full md:w-auto">
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari Kode, Nama, Barang..."
                class="px-4 py-2 border rounded-lg text-sm focus:ring-black focus:border-black w-full md:w-64">
        ...
        <div class="overflow-x-auto w-full">
            <table class="w-full min-w-[1000px] text-left text-sm whitespace-nowrap">
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($peminjaman as $codeItem)
                        @php
                            $group = \App\Models\Peminjaman::where('kode', $codeItem->kode)->with(['user', 'barang', 'barangUnit'])->get();
                            $first = $group->first();
                            $totalItems = $group->sum('jumlah');
                            $itemNames = $group->pluck('barang.nama_barang')->unique()->take(3)->implode(', ');
                            if ($group->unique('barang_id')->count() > 3) {
                                $itemNames .= ', ...';
                            }

                            // Determine overall status
                            $status = $first->status; // Fallback
                            if ($group->contains('status', 'pending')) {
                                $status = 'pending';
                            } elseif ($group->contains('status', 'approved')) {
                                $status = 'approved';
                            } elseif ($group->contains('status', 'active')) {
                                $status = 'active';
                            }
                        @endphp
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4">
                                <span class="font-mono text-xs font-bold text-gray-900">{{ $codeItem->kode ?? '-' }}</span>
                            </td>
                            <td class="px-6 py-4">
                                <div class="font-medium text-gray-900">{{ $first->user->nama_lengkap ?? '-' }}</div>
                                <div class="text-xs text-gray-500">
                                    {{ $first->user->siswa->kelas->nama_kelas ?? ucfirst($first->user->role ?? '-') }}
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="font-medium">{{ $itemNames }}</div>
                                <div class="text-xs text-gray-500">
                                    Total: {{ $group->count() }} Jenis Barang
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <div>{{ \Carbon\Carbon::parse($first->tgl_pinjam)->format('d/m/Y') }}</div>
                                <div class="text-xs text-gray-400">s/d
                                    {{ \Carbon\Carbon::parse($first->tgl_kembali_rencana)->format('d/m/Y') }}</div>
                            </td>
                            <td class="px-6 py-4">
                                <span class="font-bold text-gray-900">{{ $totalItems }}</span> Unit
                            </td>
                            <td class="px-6 py-4 text-gray-500 max-w-xs truncate" title="{{ $first->tujuan_pinjam }}">
                                {{ Str::limit($first->tujuan_pinjam, 30) }}
                            </td>
                            <td class="px-6 py-4">
                                @php
                                    $colors = [
                                ...
                                @endphp
                                <span
                                    class="px-2 py-1 rounded-md text-xs font-semibold {{ $colors[$status] ?? 'bg-gray-100' }}">
                                    {{ ucfirst($status) }}
                                </span>
                            </td>
                            <td class="px-6 py-4 text-right">
                                <a href="{{ route('admin.peminjaman.show', $first->id) }}"
                                    class="text-gray-500 hover:text-black transition text-sm font-medium">
                                    Detail
                                </a>
                            </td>
                        </tr>
