...
@section('content')
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6 lg:mb-8">
        ...
        <span class="hidden lg:inline-flex px-3 py-1 bg-black text-white text-sm font-medium rounded-full">
    ...
    <!-- Quick Actions -->
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 lg:gap-6 mb-6 lg:mb-8">
        <a href="{{ route('admin.scan.index') }}"
            ...
            <h3 class="font-semibold text-gray-900 text-sm lg:text-base mb-1">Scan QR</h3>
            <p class="text-xs lg:text-sm text-gray-500">Scan bukti peminjaman</p>
        </a>
            ...
            @forelse($recentActivities as $activity)
                <div class="flex items-start gap-4 p-4 rounded-xl bg-gray-50 hover:bg-gray-100 transition">
                    <div
                        ...
                        {{ substr($activity->user->nama_lengkap ?? $activity->user->email ?? 'A', 0, 1) }}
                    </div>
                    <div class="flex-1">
                        <p class="text-sm font-medium text-gray-900">{{ $activity->description }}</p>
                        <p class="text-xs text-gray-500 mt-1">
                            Oleh: <span class="font-medium">{{ $activity->user->nama_lengkap ?? $activity->user->email }}</span>
    ...
    <div class="bg-white rounded-xl lg:rounded-2xl border border-gray-100 p-4 lg:p-6 mb-6">
        <div class="flex items-center justify-between mb-4">
            ...
            <a href="{{ route('admin.peminjaman.index') }}" class="text-sm text-blue-600 hover:text-blue-700">Lihat
                Semua</a>
        </div>
    ...
    <div class="bg-white rounded-xl lg:rounded-2xl border border-gray-100 p-4 lg:p-6">
        <div class="flex items-center justify-between mb-4">
            ...
            <a href="{{ route('admin.pengaduan.index') }}" class="text-sm text-blue-600 hover:text-blue-700">Lihat Semua</a>
        </div>
        <div class="overflow-x-auto">
            <table class="w-full min-w-[600px] text-left text-sm">
                <thead>
    ...
    <script>
        ...
        function updateChart(filter) {
            ...
            // Fetch Data
            fetch(`{{ route('admin.dashboard.chart') }}?filter=${filter}`)
