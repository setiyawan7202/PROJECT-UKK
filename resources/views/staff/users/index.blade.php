...
@section('content')
    ...
    <div class="bg-white rounded-xl lg:rounded-2xl border border-gray-100 p-4 mb-4">
        <form method="GET" action="{{ route('staff.users.index') }}" class="flex flex-col lg:flex-row gap-4">
            <!-- Search -->
            <div class="flex-1">
                <div class="relative">
            ...
            <div class="flex flex-wrap gap-2 items-center">
                <a href="{{ route('staff.users.index', ['search' => $search ?? '']) }}"
                    class="px-4 py-2 rounded-lg text-sm font-medium transition {{ !$statusFilter && !$kelasFilter ? 'bg-black text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200' }}">
                    Semua
                </a>
                <span class="border-l border-gray-300 mx-1"></span>
                <a href="{{ route('staff.users.index', ['status' => 'siswa', 'search' => $search ?? '']) }}"
                ...
                <a href="{{ route('staff.users.index', ['status' => 'guru', 'search' => $search ?? '']) }}"
                ...
                <span class="border-l border-gray-300 mx-1"></span>
                <select name="kelas"
                    onchange="window.location.href='{{ route('staff.users.index') }}?kelas=' + this.value + '&search={{ $search ?? '' }}'"
        ...
        <div class="overflow-x-auto w-full">
            <table class="w-full min-w-[1000px] text-left text-sm whitespace-nowrap">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    @empty
                        <tr>
                            <td colspan="{{ Auth::user()->hasPermission('manage_users') ? '8' : '7' }}"
